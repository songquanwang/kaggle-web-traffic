...
from tensorflow.contrib.cudnn_rnn import CudnnGRUSaveable
...
def make_encoder_bkp(time_inputs, encoder_features_depth, is_train, hparams, seed, transpose_output=True):
    """
    ...
    # model = partial(cuda_model, inputs=(hparams.encoder_rnn_layers, tf.shape(time_inputs)[0], hparams.rnn_depth), initial_state=input_h)
    model = partial(cuda_model, inputs=rnn_time_input)
...
def make_encoder(time_inputs, encoder_features_depth, is_train, hparams, seed, transpose_output=True):
    """
    cudnn无法运行，用 dynamic_rnn 替代
    :param time_inputs:
    :param encoder_features_depth:
    :param is_train:
    :param hparams:
    :param seed:
    :param transpose_output:
    :return:
    """
    ...
    def build_init_state():
        batch_len = tf.shape(time_inputs)[0]
        return tuple(hparams.encoder_rnn_layers * [tf.zeros([batch_len, hparams.rnn_depth], dtype=tf.float32)])

    input_h = build_init_state()
    time_first = tf.transpose(time_inputs, [1, 0, 2])
    rnn_time_input = time_first
    if RNN == tf.contrib.cudnn_rnn.CudnnLSTM:
        rnn_out, rnn_state, c_state = tf.nn.dynamic_rnn(cell, rnn_time_input, initial_state=input_h, time_major=True)
    else:
        with tf.variable_scope('encoder_cell'):
            rnn_out, rnn_state = tf.nn.dynamic_rnn(cell, rnn_time_input, initial_state=input_h, time_major=True)
            rnn_state = tuple([tf.stack(rnn_state)])
            c_state = None
    if transpose_output:
        rnn_out = tf.transpose(rnn_out, [1, 0, 2])
    return rnn_out, rnn_state, c_state
...
def make_fingerprint(x, is_train, fc_dropout, seed):
    ...
    例如：x=(batch,283,5)----(batch,283,16)对不同时间的序列进行一维卷积
    卷积完后，生成 （batch，1600）全连接---（batch，16）
    :param x:
    :param is_train:
    :param fc_dropout:
    :param seed:
...
def attn_readout_v3(readout, attn_window, attn_heads, page_features, seed):
    ...
    :param readout: [n_days, batch, readout_depth] --(283,?,64)
    :param attn_window: 64
    :param attn_heads: 1
    :param page_features: (?,16)
    :param seed: 5
    :return:
    """
    # input: [n_days, batch, readout_depth] --(283,?,64)
    # [n_days, batch, readout_depth] -> [readout_depth, width=n_days, channels=batch]
    # (64,283,?)
    ...
    # [batch, attn_window * n_heads] =(?,221*1)
    ...
    # [batch, attn_window, n_heads] -> [width(attn_window), channels(batch), n_heads] ==(221,?,1)
    attns_max = tf.transpose(attns_max, [1, 0, 2])

    # [width(attn_window), channels(batch), n_heads] -> [height(1), width(attn_window), channels(batch), multiplier(n_heads)] =(1,221,?,1)
    attn_filter = attns_max[tf.newaxis, :, :, :]
    # [batch(readout_depth), height=1, width=n_days, channels=batch] -> [batch(readout_depth), height=1, width=predict_window, channels=batch*n_heads] --(64,1,63,?)
    averaged = tf.nn.depthwise_conv2d_native(inp, attn_filter, [1, 1, 1, 1], 'VALID')
    # [batch, height=1, width=predict_window, channels=readout_depth*n_neads] -> [batch(depth), predict_window, batch*n_heads] (64,63,?)
    attn_features = tf.squeeze(averaged, 1)
    # [batch(depth), predict_window, batch*n_heads] -> [batch*n_heads, predict_window, depth] (?,63,64)
...
def make_train_op(loss, ema_decay=None, prefix=None):
    ...
    # Clip gradients  计算梯度 27个需要优化的tensor
    grads_and_vars = optimizer.compute_gradients(total_loss)
    gradients, variables = zip(*grads_and_vars)
    # 修正梯度 根据L2范数调整
    clipped_gradients, glob_norm = tf.clip_by_global_norm(gradients, GRAD_CLIP_THRESHOLD)
    # apply_gradients() 函数来更新该梯度所对应的参数的状态。
    sgd_op, glob_norm = optimizer.apply_gradients(zip(clipped_gradients, variables)), glob_norm

    # Apply SGD averaging00 = {tuple} <class 'tuple'>: (<tf.Tensor 'm_0/gradients/m_0/cudnn_gru/CudnnRNN_grad/tuple/control_dependency_3:0' shape=<unknown> dtype=float32>, <tf.Variable 'm_0/cudnn_gru/opaque_kernel:0' shape=<unknown> dtype=float32_ref>)
    if ema_decay:
...
def convert_cudnn_state_v2(h_state, hparams, seed, c_state=None, dropout=1.0):
    ...
    def squeeze(seq):
        # list 转tuple ;如果只有一个元素，则取出来
        return tuple(seq) if len(seq) > 1 else seq[0]

    def wrap_dropout(structure):
        # structure是个多层数组，对每一层dropout,当前只有一层 返回list[(1,?,267)]
        if dropout < 1.0:
    ...
    # (1,?,267) == (1,batch,features)  h_state是一个tuple(((1,?,267)),((1,?,267))) 因为只有rnn只有一层，所有需要unstack，获取每一层的数组
...
def rnn_stability_loss(rnn_output, beta):
    ...
    if beta == 0.0:
        return 0.0
    # [time, batch, features] -> [time, batch] (283 ? 267)--(283,?)
...
class Model:
    def __init__(self, inp: InputPipe, hparams, is_train, seed, graph_prefix=None, asgd_decay=None, loss_mask=None):
        ...
        # Calculate attention vector (?,63,64) None
        ...
        # Get final denormalized predictions 把标准化结果转成 非标准化预测结果，同时转置成（batch,predict_list)
    ...
    def decoder(self, encoder_state, attn_features, prediction_inputs, previous_y):
        ...
        # [batch_size, time, input_depth] -> [time, batch_size, input_depth] (63,?,23) 改成时间维度在前，跟训练保持一致
        ...
        encoder_state = tf.squeeze(encoder_state, axis=0)  # 否则报错 （?,267)
        ...
        # Get final tensors from buffer arrays （63，batch，1）
