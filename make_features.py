...
def read_all() -> pd.DataFrame:
    ...
    if os.path.exists(path):
        df = pd.read_pickle(path).iloc[0:1000,:]
    else:
        # Official data
...
def prepare_data(start, end, valid_threshold) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    # boolean mask for bad (too short) series  一维array
...
def run():
    ...
    parser.add_argument('--valid_threshold', default=0.0, type=float,
    ...
    # 二维数组；每行代表唯一网页， 四个元素代表 四中代理，值保存url的index
    ...
    # Assemble indices for quarterly lagged data  序号：0--866  3 6 9 12 个月之前的日期序号
...
def run_local():
    ...
    quarter_autocorr = normalize(np.nan_to_num(raw_quarter_autocorr)) # -4.5 ,+4.5

    # Calculate and encode page features  页面url没有变化，提取了三个特征 agent country  site
    page_features = make_page_features(df.index.values)
    encoded_page_features = encode_page_features(page_features) # 10555 + 127181 + 7300 =145036
